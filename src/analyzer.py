...
def resolve_analysis_max_output_tokens() -> int:
    """Output budget for the single-stock analysis call.

    Reasoning models (deepseek-v4-*) emit ``reasoning_content`` that shares the
    same ``max_tokens`` budget as the answer. When the budget is too small the
    thinking chain consumes all of it and the API returns an empty message,
    which surfaces as ``All LLM models failed ... empty response``. The default
    is therefore generous enough for a full thinking chain plus the JSON answer.
    """
    return _env_int("LLM_ANALYSIS_MAX_OUTPUT_TOKENS", 16384, minimum=1024)


def resolve_thinking_override() -> Optional[Dict[str, Any]]:
    """Return an explicit thinking-mode override from env, or None.

    Set ``LLM_THINKING=disabled`` to switch reasoning models into non-thinking
    mode. That removes the shared-budget failure mode entirely and is much
    faster, because no reasoning tokens are produced at all.
    """
    raw = (os.getenv("LLM_THINKING", "") or "").strip().lower()
    if raw in {"disabled", "disable", "false", "0", "off", "none", "non_thinking"}:
    ...
    def analyze(
        ...
        try:
            ...
            generation_config = {
                ...
                "max_output_tokens": resolve_analysis_max_output_tokens(),
            }
